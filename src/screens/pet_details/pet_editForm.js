...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], petId, pet, numfields;

        pet = row['pet'];
        petId = row['pet'].find((x) => x.type === 'keyid').value;
            ...
            if (changes.length > 0) {
                ...
                if (tmp.length > 0) {
                    ...
                    if (rslt.status) {
                        ...
                        if (Helper.IsNullValue(mapItem.navpropname)) petId = rslt.id;
        ...
        window.AlertPopup("success", "Pet is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
