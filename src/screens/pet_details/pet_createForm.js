...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, petId, numfields;
        const mapItems = MapItems;

        let pet = row['pet'];

        numfields = Helper.GetAllNumberFields(pet);
        if (numfields.length > 0) Helper.UpdateNumberFields(pet, numfields);


        // Add Or Update Pet
        rslt = await Support.AddOrUpdatePet(pet, []);
        if (rslt.status) {
            petId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Pet and navigation entity if it is deos not exist
            let navItem = pet.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "PetId", value: parseInt(petId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdatePet(data);
        ...
        window.AlertPopup("success", "Pet is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
